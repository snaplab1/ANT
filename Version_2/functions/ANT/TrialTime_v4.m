% Updated for Invalid an Valid trials 
% by Michele Maslowski on 6.18.2025
...
saveGroupData = 1;
    switch whichStudy
        case 'Behav'
            groupDir = '/Users/Shared/ANT/ANT_update_8_14_24/Behavioral/ANT/Data/group';
        %case 'MRI'
           % groupDir = '/Users/ggurariy/Dropbox/CI_Testing/fMRI/ANT/Data/group/';
    end

% Subject & scan info
 SubjID   = {'SP_t1'};     % Behavior  
...
switch whichStudy
    ...
    case 'Behav'
        DataFolder  = ('/Users/Shared/ANT/ANT_update_8_14_24/Behavioral/ANT/Data/SP_t1');
end
...
CueValidity = 8;        % Cue validity invalid, valid, center or no cue
CuePosition = 9;        % position of the invalid cue
    ...
    for d = ScanDays
        ...
        for r = 1:Nruns
           ...
           cd(BHdata_dir);   
            ...
            if strcmp(SubjID{s},'MMG') && d == 2 
                for accu = 1:numel(response)
                    if strcmp(trialch(accu),'R') && strcmp(trialprop(accu,3),'R')
                        response(1,accu) = 1;
                    elseif strcmp(trialch(accu),'L') && strcmp(trialprop(accu,3),'L')
                        response(1,accu) = 1;
                    else
                        response(1,accu) = 0;
           ...
           disp(strcat('run',num2str(r),':', num2str(numel(find(response==1))/numel(find(trialtm>0)))))
            ...
            IC{d}{r,:} = find(~cellfun(@isempty,strfind(trialprop(:,CueValidity),'I'))); %Invalid Cue
            VC{d}{r,:} = find(~cellfun(@isempty,strfind(trialprop(:,CueValidity),'V'))); % Valid Cue
            ...
            IC_CT{d}{r,:} = find(~cellfun(@isempty,strfind(trialprop(:,CueValidity),'I')) & ~cellfun(@isempty,strfind(trialprop(:,TarCol),'C')));
            IC_IT{d}{r,:} = find(~cellfun(@isempty,strfind(trialprop(:,CueValidity),'I')) & ~cellfun(@isempty,strfind(trialprop(:,TarCol),'I')));
            VC_CT{d}{r,:} = find(~cellfun(@isempty,strfind(trialprop(:,CueValidity),'V')) & ~cellfun(@isempty,strfind(trialprop(:,TarCol),'C')));
            VC_IT{d}{r,:} = find(~cellfun(@isempty,strfind(trialprop(:,CueValidity),'V')) & ~cellfun(@isempty,strfind(trialprop(:,TarCol),'I')));
            
             
            % Find Spatial cue & Congruent trials, for subject exclusion
            indices = [SC_CT{d}{r,:}];  % unwraps the cell row into an array
            SC_CT_store = horzcat(SC_CT_store, response(indices));
            ...
            Conditions = {"SC","CC","NC","IT","CT","SC_CT","CC_CT","NC_CT","SC_IT","CC_IT","NC_IT", "IC", "VC", "IC_CT", "IC_IT", "VC_CT", "VC_IT"}; % Labels for conditions
            for cc = 1:numel(Conditions)                                                             % Loop through conditions
                ...
                RTs.(Conditions{cc}){d}=horzcat(RTs.(Conditions{cc}){d},trialtm(RT_idx));  
                % Store RTs in single matrix
...
%                 end
                ...
                % Set labels
                Conditions = {"SC","CC","NC","IT","CT", "VC", "IC"};      % Labels for conditions
...
%                 end
                ...
                Conditions = {"SC","CC","NC","IT","CT", "VC", "IC"};  
            ...
            %if TxtGenDur
                ...
                for c = 1:numel(Conditions)
                    ...
                    TrialIdxs = [TrialIdxs{:}];
                    %TrialIdxs(ismember(LoopStore{d}(r,:),Trials2Rem{d}{r})) = [];         % Remove indices of bad trials
                    trial_row = [LoopStore{d}{r,:}];               % Unwrap cell row into numeric
                    bad_trials = [Trials2Rem{d}{r}];               % Also unwrap if needed
                    trial_row(ismember(trial_row, bad_trials)) = [];  % Remove bad trials
                    TrialIdxs = trial_row; 
                    %trial_row = [trial_row{:}];
...
%        export_fig(strcat(upper(SubjID{1,s}),'_ANT'), Fmt, qOpt, mOpt);
 ...
 % convert the RTs struct to a table
fields = fieldnames(RTs);
numFields = numel(fields);

% Determine the maximum length of all RT arrays
maxLen = 0;
for i = 1:numFields
    len = numel(RTs.(fields{i}){1});
    if len > maxLen
        maxLen = len;
    end
end
...
% Fill in each column
for i = 1:numFields
    data = RTs.(fields{i}){1};         % Extract the [1×N double] from the cell
    data = data(:);                    % Ensure it's a column
    % Pad with NaNs to match maxLen
    if numel(data) < maxLen
        data(end+1:maxLen) = NaN;
    end
    RTs_table.(fields{i}) = data;      % Add as table column
end
 ...
 if saveGroupData
    cd(groupDir)
    save([SubjID{1}, '_Data.mat'], 'RT_Subj', 'AccuracyStore', 'RTs')
